...
```{r}
library(readxl)
ice_data <- read_excel(file_name, sheet = 2)
```
...
head(sunapee) #just shows the first six rows to see if it filtered right
nrow(sunapee) #just shows how many rows of data I have
```
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  geom_point() +
  labs(
    title = "Ice-off Day of Year Over Time at Lake Sunapee",
    x = "Year",
    y = "Ice-Off (DOY)"
  )
```
...
**Answer 2:**
I expect the slope to be negative. If we suspect temperatures have been warming slowly over time, then ice should melt earlier in the year. This should mean the ice-off day of year would decrease which would show a negative slope. 
...
**Answer 3:**
The slope is negative. That means the ice is melting earlier as the years go on. This is what I expected because if it’s getting warmer, the ice would melt sooner each year.
...
**Answer 4:**
The R^2 for the regression is 0.2055. This means year explains about 20.5% of the changes in ice-off DOY, so year and ice_off_doy are not strongly related, but there is still a noticeable trend.
...
**Answer 6:**
The residuals look centered around zero and are roughly bell-shaped. They are not perfectly normal, but they are close enough that the linear regression assumptions are reasonably met.
```{r}
...
**Answer 7:**
The p-value for the slope is 2.504 × 10⁻⁹. This is an extremely small value, indicating that the slope is statistically different from zero.
...
**Answer 8:**
The slope is about −0.106 days per year. A 1-week change is 7 days, so it would take about 7 / 0.106 ≈ 66 years to shift by one week. I think this is meaningful because a one-week shift over several decades is a noticeable long-term change.
...
**Answer 9:**
I feel pretty comfortable using this model to see the overall trend over time, but not for making very exact predictions. The slope is statistically significant, which means the change over time is real and not just random. The residuals are centered around zero, so the model is not biased. However, the R² is only 0.205, which means year does not explain most of the ups and downs in ice-off dates. Because of that, predictions for a specific future year could still be off by quite a bit.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  ...
  labs(
    title = "Lake Sunapee Ice-off DOY with Linear Regression Line",
    x = "Year",
...
**Answer 11:**
The predicted ice-off day of year in 2040 is 102.66. The 5% and 95% residual quantiles are −19.43 and 14.15, respectively. Adding these to the prediction gives a 90% confidence interval of 83.23 to 116.81. The width of the confidence interval is 33.58 days.

```{r}
# Predict ice-off DOY in 2040 using y = b + m*x
pred_2040 <- fit$coefficients[1] + fit$coefficients[2] * 2040
pred_2040

# 90% CI using residual quantiles (5% and 95%)
q05 <- quantile(sunapee$resid, 0.05, na.rm = TRUE)
q95 <- quantile(sunapee$resid, 0.95, na.rm = TRUE)
q05
q95

ci_lower <- pred_2040 + q05
ci_upper <- pred_2040 + q95
ci_lower
ci_upper
...
Use the skills developed above to evaluate the question "Is the ice-off day of year changing more quickly in more recent years at Lake Sunapee?" (Answered analysis Below)
...
Yes, the ice-off day of year is changing more quickly in recent years.
Before 1970, the slope is about −0.110 days/year, and from 1970 onward the slope is about −0.302 days/year. Because the more recent slope is more negative (larger magnitude), it shows that ice-off is happening earlier faster in the more recent period.
...
ggplot(sunapee, aes(x = year, y = ice_off_doy)) +
  geom_point() +
  geom_abline(intercept = coef(fit_early)[1], slope = coef(fit_early)[2], linewidth = 1) +
  geom_abline(intercept = coef(fit_late)[1],  slope = coef(fit_late)[2], linewidth = 1) +
  labs(
    title = "Lake Sunapee Ice-off DOY: Trends Before vs After 1970",
    x = "Year",
    y = "Ice-off Day of Year (DOY)"
  )
```
